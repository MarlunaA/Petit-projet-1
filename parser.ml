...
let recherche_pack_mod u =
  ...
  match u with
    ...
    |Int(x) :: xs -> ([Int(x)], xs)
    |x :: xs ->
      if x <> LBRACE then failwith"un modulo est commencé par des parenthèses ou s'applique sur un seul élément entier"
      else cherche xs 1 []
...
let parserparla u =
  let rec parcourt u gauche = (*gauche est l'arbre gauche de la prochaine opération
	...
	let (droit, reste) = parcourt_droite xs 0 [] in (*droit est une exp, reste est xs à laquelle on a tronqué les éléments pris pour faire droit*)
